...
APPLICATION: dict[int, dict] = {
 ...
 2: dict(scope=CODE,
         ...
         gap="Run tools/control_coverage.py for the current list and the current rate — the "
             ...
             "leverage order, and the ORDER changed on 2026-08-12: the thirty-odd tools "
             "without a negative control are not equally load-bearing, and treating them as a "
             "flat backlog had put the loudest first rather than the most consequential. Eight "
             "of them are run by a landing gate or by rebuild.py — so a landing's green already "
             "depends on them — and two of those eight were the corpus's own integrity tools. "
             "**build_manifest.py and validate_provenance.py now have negative controls**, "
             "asserted through the effect-boundary harness against a fresh copy: a modified "
             "anchored artifact, a rewritten manifest lineage, a missing artifact, an unreadable "
             "target, an unparseable artifact and a wrong source hash each refuse AND leave the "
             "image byte-identical. validate_provenance.py also stopped raising a traceback on a "
             "target it could not read; that is now a typed UNREADABLE failure. **The gated set "
             "did not shrink**, because the same review that ranked it found the measure itself "
             "counting prose as evidence — build_viewer.py and test_integrity.py were scored "
             "covered on a comment and a rendered page, and dropping them back to NONE put two "
             "more into the gated eight as the two integrity tools left it (D-68). The rate went "
             "DOWN when the measure got honest, which is the correct direction for that news. "
             "**Every determination is now a DECLARATION, and the gate is wired.** The "
             "600-character proximity heuristic is gone: each of its 21 determinations was read "
             "by hand, seventeen were genuine and are declared in the suite that exercises them, "
             "and four were not — one matched a `shutil.copy` list of file names, one matched "
             "prose inside a check label about a different component. The rate fell 57 -> 54 -> "
             "49 percent across the day as the measure got honest, and every fall was the "
             "measure improving rather than the code decaying. `negative-control-ratchet` is "
             "land.py's eleventh gate: it refuses a lost negative control, a vanished baselined "
             "tool and NEW debt, and its success line says NON-REGRESSION ONLY beside the count "
             "still uncovered, because a green called `coverage` would certify something it does "
             "not check. "
             "*Remaining:* the ratchet is a floor and not a target — a number that may not fall "
             "is also one nobody has to raise. A tool with a real refusal case and no "
             "declaration now reads NONE, which under-reports rather than overclaims, and that "
             "is the direction to be wrong in. "
             "**The publishers came next, and the first one repaid it immediately**: "
             "build_viewer.py, which prunes docs/, had four guards standing between its pruner "
             "and a live page — in the tool whose predecessor deleted 189 of them — and none of "
             "the four had a case it must fail. The first run of the new case found that the "
             "tool rewrote four sitemaps and THEN refused (D-69). Covered now, with "
             "build_bundle.py's immutability refusal beside it. "
             "**Every tool a gate runs now has a case it must fail** — the ranked set is "
             "closed. The last two needed a decision rather than a fixture: rebuild.py's refusal "
             "is that it HALTS at a failing step, asserted over the step labels it prints as it "
             "starts each one, because a deterministic rebuild of a built repository writes "
             "nothing and a filesystem assertion would have passed either way; test_integrity.py "
             "works against a clean clone and REBUILDS inside it, so it cannot see a deleted "
             "output — its discriminating break is a source the rebuild cannot repair. "
             "build_challenge_page.py had no refusal path at all and now refuses an emptied or "
             "absent CHALLENGE.md rather than publishing a stub at the address the record tells "
             "implementers to use. "
             "*Remaining:* thirty tools outside the gate path, and the ranking that closed this "
             "set says nothing about them — a tool nothing runs automatically is not thereby "
             "safe, only unmeasured by this ordering. "
             "Then: arm_acceptance.py, which gates whether a tool-using arm's first "
